...
//Change buffers in global iostream to read/write from/to file if necessary	
void calculate() {
		...
		while (is) {
			try {
                ...
                if (t.kind == help) {
                    ...
                    //from test.txt to result.txt					
					//input
                    string ifile;
					is >> ifile;
					os << "Input file: " << ifile << endl;
					ifstream ifs(ifile);
                    if (!ifs) { error("Not possible to open file ", ifile); }

					//change iobuffers
					streambuf *osbuf = os.rdbuf();
					streambuf *isbuf = is.rdbuf();
					...
					if (t.kind == to) {
						string ofile;
						is >> ofile;
						os << "Output file: " << ofile << endl;
						ofs.open(ofile);
						if (!ofs) { error("Not possible to open file ", ofile); }
						                    
						os.rdbuf(ofs.rdbuf());
					}

					is.rdbuf(ifs.rdbuf());
                    //It is no visible new os in the scope of this function. Why?
                    //Because output file define in upper block and it is closed in the end of block
                    calculate();
					//return iobuffers
                    is.rdbuf(isbuf);
                    os.rdbuf(osbuf);
                    os << "File reading finished." << endl;
                    continue;
                }
